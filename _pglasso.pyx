...
cdef class CSimplePathwayGeneral(object):
    cdef public double[:, ::1] Theta, shift, S, Z, V, W, Told, U, my_slice
    ...
    def __init__(self, S, Z, V, int pathway_id, pathway, neighbours, int n_pathways, Theta = None, debug=False):
        ...
        self.Z = Z[np.ix_(self.pathway, self.pathway)].copy()
        self.V = V[np.ix_(self.pathway, self.pathway)].copy()
        self.W = self.S.copy()
    ...
    cpdef double local_update_params(self, double rho, double nk, double tol, component):
        cdef int i, j
        if self.debug:
            for kk in component:
        ...
        theta_updating(self.S, self.Z, self.V, self.shift, self.Theta, nk, rho)
        #new_py_dpglasso_shift(self.S, rho, self.shift, X=self.Theta, invX=self.W, U=self.U, outer_tol=tol/2.5, outerMaxiter=75, check_nan=True)
        if self.debug:
...
def setup_pathways(graph, pathways, S, Z, V, Theta=None, debug=False, full_setup=False):
    n = len(pathways)
    objs = []
    ...
    for i in xrange(n):
        ...
        if skip:
            ...
            new = CSimplePathwayGeneral(S, Z, V, i, pathways[i], neibs, len(pathways), Theta=Theta, debug=debug)
            objs.append(new)
    return objs
...
cpdef double do_one_pass_DC_inner_recursive2(list objs, list order, double rho, double nk, double tol, list component, int t, int msg):
        # setup:
        cdef int i, j
        ...
        return do_one_pass_DC_inner2(objs, order[t:], rho, nk, tol, component, msg)

cpdef double do_one_pass_DC_inner2(list objs, list order, double rho, double nk, double tol, list component, int msg):
    ...
    if len(order) == 1:
        ...
        if msg > 0:
            ...
            #print ".",
            #sys.stdout.flush()
        res = obj.local_update_params(rho, nk, tol, component)
        for i in component:
            if i != k:
                obj2 = objs[i]
                obj2.sync_from(obj)
        ...
        tmp1 = do_one_pass_DC_inner_recursive2(objs, order, rho, nk, tol, component, t, msg)


        order = list(reversed(order))
        t = len(order) - t

        tmp2 = do_one_pass_DC_inner_recursive2(objs, order, rho, nk, tol, component, t, msg)


        return tmp1 + tmp2

cpdef double do_one_pass_DC_inner_recursive(list objs, list order, double rho, double nk, double tol, list component, int t, int msg):
        # setup:
        cdef int i, j, N
        ...
        cdef double result = do_one_pass_DC_inner(objs, order[t:], rho, nk, tol, component, msg)
...
cpdef double do_one_pass_DC_inner(list objs, list order, double rho, double nk, double tol, list component, int msg):
    cdef CSimplePathwayGeneral obj, obj2
    cdef int i, k
    cdef double tmp1, tmp2
    if len(order) == 1:
        ...
        tmp1 = do_one_pass_DC_inner_recursive(objs, order, rho, nk, tol, component, t, msg)


        order = list(reversed(order))
        t = len(order) - t

        tmp2 = do_one_pass_DC_inner_recursive(objs, order, rho, nk, tol, component, t, msg)


        return tmp1 + tmp2

def cpglasso_DC(S, Z, V, pathways, rho, nk, tol=1e-4, maxIter=50, debug=False, Theta=None, int msg=2):
    pathways = map(lambda x: list(sorted(x)), pathways)
    graph = get_pathway_graph(pathways)
    tree = nx.minimum_spanning_tree(graph)
    objs = setup_pathways(graph, pathways, S, Z, V, Theta=Theta, debug=debug)
    j = 1
    ...
    for component in components:
        ...
        for i in xrange(maxIter):
            ...
            err = do_one_pass_DC_inner2(objs, order, rho, nk, tol, order, msg=msg)
            if i % 2 == 1:
                gc.collect()
            if msg > 0:
                pass
...
@cython.wraparound(True)
def update_pathway(objs, tree, pathway_sets, pindex, nk, rho=0.1, update=True, tol=1e-6, debug=False):
    for obj in objs:
        obj.clear_messages()
    ...
    res = -1
    if update:
        res = objs[pindex].local_update_params(rho, nk, tol, range(len(objs)))
...
def cpglasso(S, pathways, rho, nk, tol=1e-4, random_trees=False, debug=False, maxIter=25):
    """
    > cpglasso(S, pathways, rho, tol=1e-4, random_trees=False, debug=False, maxIter=25)
    S - covariance matrix
    ...
    for i in xrange(maxIter):
        ...
        for j in xrange(k):
            err += update_pathway(objs, tree, pathways,  j, rho=rho, tol=tol)
        print err
        if err < tol:
            break
